...
impl ChunkTestFixture {
    pub fn new(
        ...
        let epoch_manager =
            make_epoch_manager(store, num_block_producers, num_chunk_only_producers, num_shards, 2);
...
pub fn make_epoch_manager(
    ...
    let chunk_only_producers =
        (0..num_chunk_only_producers).map(|i| format!("test_cop_{}", i).parse().unwrap()).collect();
