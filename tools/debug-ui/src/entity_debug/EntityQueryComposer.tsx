    ...
    const submitQuery = useCallback(() => {
        if (constructedQuery && fetcher) {
            allQueriesDispatch({
                ...
                node: new EntityDataRootNode(constructedQuery, coldStorage, fetcher.fetch(constructedQuery, coldStorage)),
            });
        }
            ...
            <div className="list-body">
                {entityQueryTypes.map((queryType) => {
                    return (
                        <div
                            key={queryType}
                            className={`list-item ${queryType == query.queryType ? 'list-item-selected' : ''
                                }`}
