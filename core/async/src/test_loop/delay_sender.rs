...
pub struct DelaySender<Event> {
    ...
    // traits much easier.
    im: Arc<dyn DelayCanSend<Event>>,
}

pub(crate) trait DelayCanSend<Event>: Send + Sync {
    fn send_with_delay(&self, event: Event, delay: Duration);
}

/// Implements any DelayCanSend<Other> as long as Other can be converted into Event.
pub struct NarrowingDelaySender<Event> {
    sender: DelaySender<Event>,
}

impl<Event, OuterEvent: From<Event>> DelayCanSend<Event> for NarrowingDelaySender<OuterEvent> {
    fn send_with_delay(&self, event: Event, delay: Duration) {
...
impl<Event> DelaySender<Event> {
    pub(crate) fn new<Impl: DelayCanSend<Event> + 'static>(im: Impl) -> Self {
        Self { im: Arc::new(im) }
    }
