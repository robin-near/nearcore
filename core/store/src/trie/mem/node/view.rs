use super::{MemTrieNodePtrGeneric, MemTrieNodeViewGeneric};
...
impl<'a, Memory: IArenaMemory> MemTrieNodeViewGeneric<'a, Memory> {
    ...
    pub(crate) fn iter_children<'b>(
        &'b self,
    ) -> Box<dyn Iterator<Item = MemTrieNodePtrGeneric<'a, Memory>> + 'b> {
        match self {
